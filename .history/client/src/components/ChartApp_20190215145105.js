...
var CanvasJSReact = require('../components/canvasjs/canvasjs.react');
...
class ChartApp extends Component {
  ...
  componentDidMount(){
    axios.get('http://localhost:4242/poll')
         .then((res) => {
            ...
            const votesCounts =  votes.reduce((acc, vote) => (
  ...
  chartCanvas(votesCounts){
    ...
    if(chartContainer){
      ...
      chart.render();
      this.pusherLogic(dataPoints, chart)
    }
  }

  pusherLogic(dataPoints, chart){
